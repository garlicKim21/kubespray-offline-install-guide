...
| 2 | `kubespray_version` 업데이트 (`v2.28.x` → `v2.29.1`) | ☐ |
| 3 | 새 버전 파일 서버 업로드 | ☐ |
| 4 | 새 버전 이미지 프록시 확인 | ☐ |
| 5 | **오프라인 리소스 검증 완료** (`verify-offline-resources.yml`) | ☐ |
| 6 | **연결성 테스트 완료** (`facts.yml`) | ☐ |
...
**원인:** `--check` 모드는 실제 파일을 다운로드하지 않으므로, 복사 태스크에서 소스 파일을 찾을 수 없습니다. 이는 Ansible `--check` 모드의 본질적인 한계로, 이전 태스크의 결과에 의존하는 후속 태스크에서 발생합니다.

**해결:** 이는 예상된 동작입니다. `--check` 대신 다음 검증 방법을 권장합니다:

```bash
# 1. 오프라인 리소스 검증 (파일 서버, 레지스트리 확인)
ansible-playbook ../../../playbooks/verify-offline-resources.yml \
  -e kube_version=1.33.7

# 2. 문법 검사
ansible-playbook upgrade-cluster.yml --syntax-check \
...
자세한 내용은 [업그레이드 가이드의 사전 검증 섹션](../01-upgrade-guide.md#33-사전-검증-권장)을 참조하세요.
